...
// ── Init & persistence ────────────────────────────────────────────────────────
function init() {
  // locateFile tells sql.js where to find its .wasm binary
  // This is critical in Railway/Docker containers where __dirname resolves differently
  // sql.js needs explicit path to its .wasm binary — critical in Railway/Docker
  const wasmPath = require("path").join(
    require.resolve("sql.js"),  // resolves to node_modules/sql.js/dist/sql-wasm.js
    "..",                        // node_modules/sql.js/dist/
    "sql-wasm.wasm"
  );
  const sqlJsConfig = { locateFile: () => wasmPath };
...
function migrateSchema() {
  ...
  const migrations = [
    ...
    "ALTER TABLE user_profiles ADD COLUMN email_verified INTEGER NOT NULL DEFAULT 0",
    "ALTER TABLE user_profiles ADD COLUMN phone_verified INTEGER NOT NULL DEFAULT 0",
    "ALTER TABLE user_profiles ADD COLUMN kyc_complete INTEGER NOT NULL DEFAULT 0",
    "ALTER TABLE vendors ADD COLUMN buy_rate REAL",
    "ALTER TABLE vendors ADD COLUMN sell_rate REAL",
    "ALTER TABLE vendors ADD COLUMN pairs_traded TEXT",
    "ALTER TABLE vendors ADD COLUMN platforms_active TEXT",
    "ALTER TABLE vendors ADD COLUMN order_min REAL",
    "ALTER TABLE vendors ADD COLUMN order_max REAL",
    "ALTER TABLE vendors ADD COLUMN completion_rate REAL",
    "ALTER TABLE vendors ADD COLUMN trade_count INTEGER",
...
const Products = {
  ...
  searchByName: (bId, n) => all("SELECT * FROM products WHERE business_id=? AND active=1 AND LOWER(name) LIKE LOWER(?)", [bId, "%" + n + "%"]),
  add(bId, d) {
    const r = run(
...
const Expenses = {
  ...
  breakdown:   (bId) => all("SELECT category, SUM(amount) as total FROM expenses WHERE business_id=? AND strftime('%Y-%m',created_at)=strftime('%Y-%m','now') GROUP BY category ORDER BY total DESC", [bId]),
};
...
const StockMovements = {
  add(bId, d) {
    ...
    // Update product stock: "in" adds, "out" subtracts
    if (d.product_id) {
      const delta = d.type === "out" ? -Math.abs(d.quantity) : Math.abs(d.quantity);
      run("UPDATE products SET stock=MAX(0, stock+?) WHERE id=?", [delta, d.product_id]);
    }
    save(); return r;
  },
...
const Vendors = {
  ...
  update(id, fields) {
    ...
    // Crypto & Gift Cards fields
    if (fields.buy_rate    !== undefined) { sets.push("buy_rate=?");          vals.push(fields.buy_rate); }
    if (fields.sell_rate   !== undefined) { sets.push("sell_rate=?");         vals.push(fields.sell_rate); }
    if (fields.pairs_traded)     { sets.push("pairs_traded=?");     vals.push(fields.pairs_traded); }
    if (fields.platforms_active) { sets.push("platforms_active=?"); vals.push(fields.platforms_active); }
    if (fields.order_min   !== undefined) { sets.push("order_min=?");         vals.push(fields.order_min); }
    if (fields.order_max   !== undefined) { sets.push("order_max=?");         vals.push(fields.order_max); }
    if (fields.completion_rate !== undefined) { sets.push("completion_rate=?"); vals.push(fields.completion_rate); }
    if (fields.trade_count !== undefined) { sets.push("trade_count=?");       vals.push(fields.trade_count); }
    if (!sets.length) return;
    vals.push(id);
    run("UPDATE vendors SET " + sets.join(",") + " WHERE id=?", vals);
    save();
  },
  // Shortcut for crypto P2P rate updates by telegram_id
  updateRates(tgId, buyRate, sellRate) {
    run("UPDATE vendors SET buy_rate=?, sell_rate=? WHERE telegram_id=?",
      [buyRate, sellRate, String(tgId)]);
    save();
  },
  updateField(tgId, field, value) {
    const allowed = ["completion_rate","trade_count","pairs_traded","platforms_active","order_min","order_max","buy_rate","sell_rate"];
    if (!allowed.includes(field)) return;
    run(`UPDATE vendors SET ${field}=? WHERE telegram_id=?`, [value, String(tgId)]);
    save();
  },
...
const UserProfiles = {
  ...
  upsert(d) {
    ...
    if (ex) {
      run("UPDATE user_profiles SET name=COALESCE(?,name),business_name=COALESCE(?,business_name),phone=COALESCE(?,phone),address=COALESCE(?,address),city=COALESCE(?,city),state=COALESCE(?,state),industry=COALESCE(?,industry),description=COALESCE(?,description),location=COALESCE(?,location) WHERE id=?",
        [d.name||null,d.business_name||null,d.phone||null,d.address||null,d.city||null,d.state||null,d.industry||null,d.description||null,d.location||null,ex.id]);
    } else {
      run("INSERT INTO user_profiles (telegram_id,email,name,business_name,phone,address,city,state,industry,plan) VALUES (?,?,?,?,?,?,?,?,?,?)",
        [d.telegram_id||null,d.email||null,d.name,d.business_name||null,d.phone||null,d.address||null,d.city||null,d.state||null,d.industry||null,d.plan||"trial"]);
    }
    save();
  },
  verifyEmail: (email) => { run("UPDATE user_profiles SET email_verified=1 WHERE email=?", [email]); save(); },
  verifyPhone: (tgId)  => { run("UPDATE user_profiles SET phone_verified=1 WHERE telegram_id=?", [String(tgId)]); save(); },
  completeKYC: (email) => {
    run("UPDATE user_profiles SET kyc_complete=1, verified=1 WHERE email=?", [email]);
    save();
  },
  linkTelegram: (email, tgId) => {
    run("UPDATE user_profiles SET telegram_id=? WHERE email=?", [String(tgId), email]);
    save();
  },
  verify: (tgId) => { run("UPDATE user_profiles SET verified=1 WHERE telegram_id=?", [String(tgId)]); save(); },
  isKYCComplete: (email) => {
    const p = get("SELECT kyc_complete FROM user_profiles WHERE email=?", [email]);
    return !!p?.kyc_complete;
  },
};
...
const OTP = {
  ...
  verifyByEmail(email, code, purpose="email_verify") {
    const row = get("SELECT * FROM otp_codes WHERE email=? AND code=? AND purpose=? AND used=0 AND expires_at>datetime('now') ORDER BY created_at DESC LIMIT 1",
      [email, code, purpose]);
    if (!row) return false;
    run("UPDATE otp_codes SET used=1 WHERE id=?", [row.id]);
    save(); return true;
  },
  generateForEmail(email, purpose="email_verify") {
    const code = Math.floor(100000 + Math.random()*900000).toString();
    const exp  = new Date(Date.now() + 15*60*1000).toISOString();
    run("INSERT INTO otp_codes (email,code,purpose,expires_at) VALUES (?,?,?,?)",
