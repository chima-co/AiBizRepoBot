  ...
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS marketplace_sales (
      ...
      source          TEXT    NOT NULL DEFAULT 'marketplace'
                              CHECK(source IN ('marketplace','external','bot')),
      created_at      TEXT    NOT NULL DEFAULT (datetime('now')),
      completed_at    TEXT
    );
...
const Signups = {
  add(data) {
    const r = run(
      "INSERT OR IGNORE INTO signups (name,business_name,email,phone,telegram_username,industry,plan) VALUES (?,?,?,?,?,?,?)",
      [data.name, data.business_name||null, data.email, data.phone||null, data.telegram_username||null, data.industry||null, data.plan||'trial']
    );
    // Auto-create vendor profile using business_name if provided
  ...
  count() { return get("SELECT COUNT(*) as total, COUNT(CASE WHEN plan='lifetime' THEN 1 END) as paid, COUNT(CASE WHEN plan='trial' THEN 1 END) as trial FROM signups")|| {total:0,paid:0,trial:0}; },
};

// ─── OTP ──────────────────────────────────────────────────────
const OTP = {
  generate(telegramId, phone, email, purpose = 'signup') {
    const code = Math.floor(100000 + Math.random() * 900000).toString();
    const expires = new Date(Date.now() + 10 * 60 * 1000).toISOString(); // 10 min
    run("INSERT INTO otp_codes (telegram_id,phone,email,code,purpose,expires_at) VALUES (?,?,?,?,?,?)",
      [telegramId||null, phone||null, email||null, code, purpose, expires]);
    save();
    return code;
  },
  verify(telegramId, code, purpose = 'signup') {
    const row = get(
      "SELECT * FROM otp_codes WHERE telegram_id=? AND code=? AND purpose=? AND used=0 AND expires_at > datetime('now') ORDER BY created_at DESC LIMIT 1",
      [String(telegramId), code, purpose]
    );
    if (!row) return false;
  ...
  verifyByPhone(phone, code, purpose = 'signup') {
    const row = get(
      "SELECT * FROM otp_codes WHERE phone=? AND code=? AND purpose=? AND used=0 AND expires_at > datetime('now') ORDER BY created_at DESC LIMIT 1",
      [phone, code, purpose]
    );
    if (!row) return false;
    run("UPDATE otp_codes SET used=1 WHERE id=?", [row.id]);
...
// ─── User Profiles ────────────────────────────────────────────
const UserProfiles = {
  get(telegramId) { return get("SELECT * FROM user_profiles WHERE telegram_id=?", [String(telegramId)]); },
  getByEmail(email) { return get("SELECT * FROM user_profiles WHERE email=?", [email]); },
  upsert(data) {
    const existing = data.telegram_id ? UserProfiles.get(data.telegram_id) : UserProfiles.getByEmail(data.email);
    if (existing) {
      run("UPDATE user_profiles SET name=?,business_name=?,phone=?,industry=?,description=?,location=?,plan=? WHERE id=?",
        [data.name||existing.name, data.business_name||existing.business_name, data.phone||existing.phone,
         data.industry||existing.industry, data.description||existing.description,
         data.location||existing.location, data.plan||existing.plan, existing.id]);
    } else {
      run("INSERT INTO user_profiles (telegram_id,email,name,business_name,phone,industry,plan) VALUES (?,?,?,?,?,?,?)",
        [data.telegram_id||null, data.email||null, data.name, data.business_name||null,
         data.phone||null, data.industry||null, data.plan||'trial']);
    }
    save();
  },
  setPlan(telegramId, plan, expiresAt) {
    run("UPDATE user_profiles SET plan=?, plan_expires_at=? WHERE telegram_id=?", [plan, expiresAt||null, String(telegramId)]);
    save();
  },
  verify(telegramId) {
    run("UPDATE user_profiles SET verified=1 WHERE telegram_id=?", [String(telegramId)]);
    save();
  },
};
...
const VendorListings = {
  add(data) {
    const r = run("INSERT INTO vendor_listings (vendor_id,business_id,title,description,price,price_label,category,image_url) VALUES (?,?,?,?,?,?,?,?)",
      [data.vendor_id, data.business_id||null, data.title, data.description||null,
       data.price||null, data.price_label||null, data.category||null, data.image_url||null]);
    save(); return r;
  },
  list(vendorId) { return all("SELECT * FROM vendor_listings WHERE vendor_id=? AND available=1 ORDER BY created_at DESC", [vendorId]); },
  listAll(limit=20) { return all("SELECT vl.*, v.name as vendor_name, v.industry, v.phone, v.location FROM vendor_listings vl JOIN vendors v ON v.id=vl.vendor_id WHERE vl.available=1 ORDER BY vl.created_at DESC LIMIT ?", [limit]); },
  get(id) { return get("SELECT vl.*, v.name as vendor_name, v.phone, v.industry FROM vendor_listings vl JOIN vendors v ON v.id=vl.vendor_id WHERE vl.id=?", [id]); },
  incrementViews(id) { run("UPDATE vendor_listings SET views=views+1 WHERE id=?", [id]); },
};
...
const MarketplaceSales = {
  create(data) {
    const r = run("INSERT INTO marketplace_sales (listing_id,seller_tg_id,buyer_tg_id,buyer_name,buyer_phone,amount,description,source) VALUES (?,?,?,?,?,?,?,?)",
      [data.listing_id||null, data.seller_tg_id, data.buyer_tg_id||null, data.buyer_name||null,
       data.buyer_phone||null, data.amount||0, data.description||null, data.source||'marketplace']);
    save(); return r;
  },
  confirmSeller(id, sellerTgId) {
    const row = get("SELECT * FROM marketplace_sales WHERE id=? AND seller_tg_id=?", [id, String(sellerTgId)]);
    if (!row) return false;
    ...
    if (updated.buyer_confirmed) {
      run("UPDATE marketplace_sales SET status='completed', completed_at=datetime('now') WHERE id=?", [id]);
    }
    save(); return true;
  },
  confirmBuyer(id, buyerTgId) {
    const row = get("SELECT * FROM marketplace_sales WHERE id=? AND buyer_tg_id=?", [id, String(buyerTgId)]);
    if (!row) return false;
    run("UPDATE marketplace_sales SET buyer_confirmed=1, status='buyer_received' WHERE id=?", [id]);
    const updated = get("SELECT * FROM marketplace_sales WHERE id=?", [id]);
  ...
  get(id) { return get("SELECT * FROM marketplace_sales WHERE id=?", [id]); },
  listBySeller(tgId) { return all("SELECT * FROM marketplace_sales WHERE seller_tg_id=? ORDER BY created_at DESC LIMIT 20", [String(tgId)]); },
  listByBuyer(tgId) { return all("SELECT * FROM marketplace_sales WHERE buyer_tg_id=? ORDER BY created_at DESC LIMIT 20", [String(tgId)]); },
  stats(tgId) {
...
module.exports = {
  ...
  OTP, UserProfiles, VendorListings, MarketplaceSales,
};
