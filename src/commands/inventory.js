...
// ─────────────────────────────────────────────
const { Products, StockMovements } = require("../db/database");
...
function registerInventory(bot) {
  ...
  bot.on("text", async (ctx, next) => {
    ...
    if (si?.step === "quantity") {
      ...
      Products.updateStock(biz.id, si.product.id, newStock);
      StockMovements.record(biz.id, si.product.id, "in", qty, "Manual stock in");
      ctx.session.stockIn = null;
      return safeReply(ctx,
        `📥 *Stock Added!*\n\n` +
    ...
    if (so?.step === "quantity") {
      ...
      Products.updateStock(biz.id, so.product.id, newStock);
      StockMovements.record(biz.id, so.product.id, "out", qty, "Manual stock out");
      ctx.session.stockOut = null;
      return safeReply(ctx,
        `📤 *Stock Removed!*\n\n` +
