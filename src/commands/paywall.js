...
const { checkAccess, createPaymentLink, PRICE, TRIAL_DAYS } = require("../services/payment");
...
async function sendPaywall(ctx, access) {
  ...
  const sub = isExpired
    ? `You've explored ShopBoss during your trial. To keep managing your business, unlock lifetime access.`
    : `This feature requires a ShopBoss license.`;

  await ctx.reply(
    `${header}\n\n${sub}\n\n` +
    `*ShopBoss Lifetime Access*\n` +
    `💰 One-time payment: *₦${Number(PRICE).toLocaleString("en-NG")}*\n` +
    `✅ Never pay again — yours forever\n` +
    `✅ All features included\n` +
    `✅ AI business insights\n` +
    `✅ Unlimited sales, inventory & staff\n` +
    `✅ Mini App dashboard\n` +
    `✅ Future updates included\n\n` +
    `Tap below to pay securely with your card, bank transfer, or USSD:`,
    {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("💳 Pay ₦125,000 — Unlock ShopBoss", "pay_now")],
        [Markup.button.callback("❓ Why should I pay?", "pay_why")],
      ]),
    }
  );
}
...
async function sendTrialBanner(ctx, daysLeft) {
  ...
  await ctx.reply(
    ...
    `Unlock lifetime access for ₦125,000 — one-time, never expires.\n\nUse /pay to upgrade anytime.`,
    {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
...
function registerPaywall(bot) {
  ...
  bot.command("pay", async (ctx) => {
    ...
    await ctx.reply(
      ...
      `*Price: ₦125,000* (one-time, never pay again)\n\n` +
  ...
  bot.command("status", async (ctx) => {
    ...
    if (access.status === "trial") {
      return ctx.reply(
        ...
        `Unlock lifetime access for ₦125,000 — one-time payment.\nUse /pay to upgrade.`,
        {
          parse_mode: "Markdown",
  ...
  bot.action("pay_why", async (ctx) => {
    ...
    await ctx.reply(
      ...
      {
        parse_mode: "Markdown",
        ...Markup.inlineKeyboard([[Markup.button.callback("💳 Pay ₦125,000 Now", "pay_now")]]),
      }
    );
  });
      ...
      try {
        ...
        await ctx.reply(
          ...
          `After payment, ShopBoss will activate automatically within seconds.`,
          {
            parse_mode: "Markdown",
            ...Markup.inlineKeyboard([
              [Markup.button.url("💳 Pay ₦125,000 Securely", link)],
