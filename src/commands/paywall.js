...
async function sendPaywall(ctx, access) {
  ...
  await ctx.reply(
    ...
    `♾️ *Lifetime* — ₦999,999 once, forever\n\n` +
    `All plans include full access to every feature.`,
    {
      ...
      ...Markup.inlineKeyboard([
        ...
        [Markup.button.callback("♾️ Lifetime — ₦999,999", "pay:lifetime")],
...
async function sendTrialBanner(ctx, daysLeft) {
  ...
  await ctx.reply(
    ...
    `Unlock lifetime access for ₦999,999 — one-time, never expires.\n\nUse /pay to upgrade anytime.`,
    {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
...
function registerPaywall(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    // KYC gate — require completed KYC before trial access
    ...
    if (!profile?.kyc_complete) {
      ...
      await ctx.reply(
        ...
        `Before you can use ShopBoss, please complete your account setup:\n\n` +
        `✅ Full name\n` +
        `✅ Email address (with OTP verification)\n` +
        `✅ Phone number\n` +
        `✅ Business address\n\n` +
        `This takes about 2 minutes and verifies your identity.`,
        {
          parse_mode: "Markdown",
          ...Markup.inlineKeyboard([
            [Markup.button.webApp("🚀 Complete KYC Now", miniUrl)],
  ...
  bot.command("pay", async (ctx) => {
    ...
    await ctx.reply(
      ...
      `*Price: ₦999,999* (one-time, never pay again)\n\n` +
  ...
  bot.command("status", async (ctx) => {
    ...
    if (access.status === "trial") {
      return ctx.reply(
        ...
        `Unlock lifetime access for ₦999,999 — one-time payment.\nUse /pay to upgrade.`,
        {
          parse_mode: "Markdown",
  ...
  bot.action("pay_why", async (ctx) => {
    ...
    await ctx.reply(
      ...
      {
        parse_mode: "Markdown",
        ...Markup.inlineKeyboard([[Markup.button.callback("💳 Pay ₦999,999 Now", "pay_now")]]),
      }
    );
  });
      ...
      try {
        ...
        await ctx.reply(
          ...
          `After payment, ShopBoss will activate automatically within seconds.`,
          {
            parse_mode: "Markdown",
            ...Markup.inlineKeyboard([
              [Markup.button.url("💳 Pay ₦999,999 Securely", link)],
