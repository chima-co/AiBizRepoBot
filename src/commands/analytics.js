...
function registerAnalytics(bot) {
  ...
  bot.command("insights", async (ctx) => {
    ...
    try {
      ...
      const text = await generateInsights(biz.id);
  ...
  bot.command("advice", async (ctx) => {
    ...
    try {
      ...
      const advice = await askAI(biz.id, "Give me 3 specific, actionable business improvements I can make this week based on my data.");
  ...
  async function handleAIQuestion(ctx, question) {
    ...
    try {
      const { askAI } = require("../services/ai");
      const answer = await askAI(biz.id, question);
