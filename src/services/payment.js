...
// Read env vars lazily so they're always current (not captured at module load time)
function getFLWSecret()  { return process.env.FLW_SECRET_KEY || ""; }
function getWebhookUrl() { return (process.env.getWebhookUrl() || "").replace(/\/+$/, ""); }
const TRIAL_DAYS  = 14;
...
async function createPaymentLink(telegramId, customerName, customerEmail, customerPhone, plan) {
  const planConfig = PLANS[plan] || PLANS.lifetime;
  const chargeAmount = planConfig.price;
  ...
  db_run(
    ...
    [String(telegramId), txRef, chargeAmount, customerName, customerEmail]
  );
  ...
  const payload = {
    ...
    amount: chargeAmount,
    currency: "NGN",
    redirect_url: `${getWebhookUrl()}/payment/callback`,
    customer: {
      email: customerEmail,
    ...
    customizations: {
      title: "ShopBoss " + planConfig.label,
      description: planConfig.description,
      logo: `${getWebhookUrl()}/logo.png`,
    },
