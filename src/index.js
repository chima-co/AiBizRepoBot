...
init().then((dbHandles) => {
  ...
  app.post("/signup", async (req, res) => {
    try {
      const { name, email, phone, telegram_username, industry, plan } = req.body;
      if (!name || !email) return res.status(400).json({ error: "Name and email required" });
      const emailRx = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRx.test(email)) return res.status(400).json({ error: "Invalid email" });
      const result = Signups.add({ name, email, phone, telegram_username, industry, plan: plan || "trial" });
      res.json({ ok: true, message: "Signup complete! Open @AiBizRepoBot on Telegram to start." });
    } catch(err) {
      console.error("Signup error:", err.message);
      if (err.message?.includes("UNIQUE")) return res.status(400).json({ error: "This email is already registered." });
